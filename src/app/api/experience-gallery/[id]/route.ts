...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  try {
    const [rows] = await (pool as any).query('SELECT * FROM experience_gallery WHERE id = ?', [id])
...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  try {
    await (pool as any).query(
      `UPDATE experience_gallery SET ${setClauses} WHERE id = ?`,
      [...values, id]
    )
...
export async function DELETE(req: NextRequest, { params }: Params) {
  ...
  try {
    await (pool as any).query('DELETE FROM experience_gallery WHERE id = ?', [id])
    return NextResponse.json({ success: true })
  } catch (err) {
    console.error(err)
